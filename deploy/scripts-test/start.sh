...
nohup java -jar \
  -Dspring.config.location=classpath:"$PROJECT_ROOT"/application-"$IDLE_PROFILE".yml,"$PROJECT_ROOT"/application-key.yml,"$PROJECT_ROOT"/application-test-db.yml,"$PROJECT_ROOT"/application-email.yml \
  -Dspring.profiles.active="$IDLE_PROFILE" \
  "$JAR_NAME" > "$APP_LOG" 2> "$ERROR_LOG" &

CURRENT_PID=$(pgrep -f "$JAR_NAME")
echo "$TIME_NOW "$0" > 실행된 PID $CURRENT_PID" >> "$DEPLOY_LOG"
